...
class CommandManagerTest < Test::Unit::TestCase
  ...
  def test_instance
    ...
    assert(!manager.instance_variable_get('@commands').empty?)
  end
  ...
  def test_register_command
    ...
    assert_equal(true, manager.instance_variable_get('@commands').keys.member?(:toto))
  end
  ...
  def test_array_finder_mocked
    ...
    assert_equal(true, manager['list'])
